...
for(var j=0;j<staticBlocks.length;j++){
    rebound=this.collidesWidth(staticBlocks[j]);
}
if(rebound!=null&&rebound.collision){
...
Ball.prototype.collidesWith=function(block){
    var nextPosition=createVector(this.position.x+this.velocity.x,this.position.y+this.velocity.y);
    ...
    var withinBottomBound=(nextPosition.y<=lowerYBound)&&(nextPosition>=lowerYBlock);
